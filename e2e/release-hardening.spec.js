...
test("safe mode and hardening diagnostics expose canonical version", async ({ page }) => {
  ...
  await expect(page.getByTestId("app-footer")).toContainText("v1.0.0-beta.5");
  ...
  expect(debug.appVersion).toBe("1.0.0-beta.5");
...
test("support bundle is downloadable and redacts API keys", async ({ page }) => {
  ...
  expect(text).toContain('"appVersion": "1.0.0-beta.5"');
