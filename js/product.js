...
export const SHOWCASE_DATASETS = [
  { file: "01_global_development_odyssey.json", title: "Global Development Odyssey", rows: 1704, description: "Historical country development indicators through 2007.", bestFor: "Line charts, vertical bars, and country point maps.", source: "Transformed Gapminder demonstration data", recipe: { title: "Population by country centroid", sql: "SELECT country, continent, centroid_lat, centroid_lon, population_millions FROM table_01_global_development_odyssey WHERE year = 2007", visualization: "Proportional-symbol point map", fields: "Latitude: centroid_lat · Longitude: centroid_lon · Size: population_millions · Color: continent", requiredColumns: ["country", "continent", "centroid_lat", "centroid_lon", "population_millions", "year"] } },
  { file: "02_montreal_mobility_constellation.json", title: "Montreal Mobility Constellation", rows: 249, description: "Montreal car-sharing availability hotspots.", bestFor: "Clustered, proportional-symbol, and category point maps.", source: "Transformed Plotly Montreal car-sharing data", tutorial: "docs/tutorials/montreal-mobility.md", recipe: { title: "Montreal availability hotspots", sql: "SELECT hotspot_id, latitude, longitude, car_hours, peak_hour, peak_period, intensity_band FROM table_02_montreal_mobility_constellation", visualization: "Category-colored point map", fields: "Latitude: latitude · Longitude: longitude · Label: hotspot_id · Color: peak_period", requiredColumns: ["hotspot_id", "latitude", "longitude", "car_hours", "peak_hour", "peak_period", "intensity_band"] } },
  { file: "03_tech_stock_time_machine.json", title: "Tech Stock Time Machine", rows: 630, description: "Demonstration technology-stock index series.", bestFor: "Symbol-specific line trends and vertical bars.", source: "Transformed Plotly stock index demonstration data", recipe: { title: "Apple index history", sql: "SELECT date, price_index FROM table_03_tech_stock_time_machine WHERE symbol = 'AAPL' ORDER BY date", visualization: "Line chart", fields: "X: date · Y: price_index", requiredColumns: ["date", "price_index", "symbol"] } },
  { file: "04_iris_morphology_lab.json", title: "Iris Morphology Lab", rows: 150, description: "Fisher iris measurements with derived morphology fields.", bestFor: "Species average vertical bars and numerical summaries.", source: "Transformed Fisher/UCI Iris data", recipe: { title: "Average petal length by species", sql: "SELECT species, AVG(petal_length) AS average_petal_length FROM table_04_iris_morphology_lab GROUP BY species ORDER BY species", visualization: "Vertical bar chart", fields: "X: species · Y: average_petal_length", requiredColumns: ["species", "petal_length"] } },
  { file: "05_wind_rose_observatory.json", title: "Wind Rose Observatory", rows: 128, description: "Wind direction and strength frequencies.", bestFor: "Direction rankings and strength-grouped vertical bars.", source: "Transformed Plotly wind demonstration data", recipe: { title: "Wind frequency by direction", sql: "SELECT direction, SUM(frequency_pct) AS frequency_pct FROM table_05_wind_rose_observatory GROUP BY direction ORDER BY frequency_pct DESC", visualization: "Vertical bar chart", fields: "X: direction · Y: frequency_pct", requiredColumns: ["direction", "frequency_pct"] } },
];
